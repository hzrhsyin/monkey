...
generateTesterIssue(issue) {
    return (
      <>
        Upgrade Tester server to versions 8.5.11, 8.6.10, or later.
        <CollapsibleWellComponent>
          Tester server at <span className="badge badge-primary">{issue.machine}</span> (<span
          className="badge badge-info" style={{margin: '2px'}}>{issue.ip_address}</span>) is vulnerable to <span
          className="badge badge-danger">remote command execution</span> attack.
          <br/>
          The attack was made possible because the server is using an old version of Tester, for which REST API is
          enabled. For possible workarounds, fixes and more info read
          <a href="https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2019-6340">here</a>.
        </CollapsibleWellComponent>
      </>
  ...
  generateIssue = (issue) => {
    ...
    switch (issue.type) {
      ...
      case 'drupal':
        ...
        break;
      case 'tester':
        issueData = this.generateTesterIssue(issue);
        break;
    }
